...
export default function AddEngAmbiguous() {
  ...
  const handleSameEngWordChange = (index, event) => {
    const newSameEngWords = [...sameEngWords];
    newSameEngWords[index] = event.target.value;
    setSameEngWords(newSameEngWords);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const newWord = { engWord, sameEngWords, sinambiWords };
  ...
  const handleEdit = (index) => {
    ...
    setSameEngWords(entries[index].sameEngWords || []);
        ...
        <table className="w-full border">
          <thead>
            <tr className="bg-green-500 text-white">
              ...
              <th className="p-2 border">Same English Words</th>
            ...
            {entries.length > 0 ? (
              entries.map((entry, index) => (
                <tr key={index} className="border-b">
                  ...
                  <td className="p-2 border">{entry.sameEngWords.join(", ")}</td>
